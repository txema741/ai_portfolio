...
def compute_kpis(df, required, issues, seg_fields, key_col=None):
    """
    KPIs robustos: no rompe si faltan columnas (p.ej., 'cliente_id') o si hay espacios en cabeceras.
    """
    # Copia y normaliza cabeceras para evitar sorpresas de espacios/bom
    df = df.copy()
    df.columns = [str(c).strip() for c in df.columns]

    # Usa la clave solo si está realmente
    safe_keycol = key_col if (key_col and key_col in df.columns) else None
    ...
    # % nulos en obligatorias (si falta una columna obligatoria, la contamos como 100% nulos)
    nulls_sum = 0
    req_list = required or []
    req_len = max(len(req_list), 1)
    for c in req_list:
        if c in df.columns:
            nulls_sum += df[c].isna().sum()
        else:
            nulls_sum += len(df)

    # % filas con duplicado de clave (solo si existe la clave)
    if safe_keycol:
        dup_rows = df.duplicated(subset=[safe_keycol], keep=False)
        pct_dups = pct(len(df[dup_rows]))
    else:
        pct_dups = 0.0

    kpi_global = {
        "%_nulos_en_obligatorias": pct(nulls_sum / req_len),
        "%_filas_con_duplicado_clave": pct_dups,
    ...
    for field in (seg_fields or []):
        ...
        for val, gdf in df.groupby(field, dropna=False):
            ...
            rows.append({
                field: str(val),
                "filas": len(gdf),
                "%_nulos_en_obligatorias": p(g_nulls_sum / req_len),
